...
<body>
  ...
  <script>
    ...
    async function saveEngines(engineIds) {
      if (!state.selectedId) return;
      try {
        state.engines = await request("/api/projects/" + encodeURIComponent(state.selectedId) + "/engines", {
    ...
    function renderEngines() {
      ...
      const rows = data.engines.map((engine) => '<div class="mrow mcols-engine">'
    ...
    document.addEventListener("change", (event) => {
      ...
      const engineBox = event.target && event.target.closest ? event.target.closest("[data-engine-checkbox]") : null;
      if (engineBox) {
