...
(() => {
  ...
  const overview = () => '<div class="p5-heading" id="p5-overview"><div><h1>Continuous measurement</h1><p class="p5-muted">A line only joins real samples from the same project, configuration, model, web search mode and protocol. Refreshing the page does not call any model.</p></div><div class="p5-actions">' + requestButton("Manage models", "models") + requestButton("Save scope", "watchset") + requestButton("Start recognition test", "run", true) + '</div></div>' + configurationSummary() + (state.error ? '<div class="p5-alert"><strong>Action did not complete</strong><p class="p5-muted">' + esc(state.error) + '</p></div>' : "") + scopeControls() + '<section class="p5-section"><h2>Current data</h2><div class="p5-grid">' + metricCards() + '</div></section><section class="p5-section" id="p5-competition"><h2>AI How answers about your brand change</h2><p class="p5-muted">Each point is one complete run. Each line is a single model. Different protocols, web search modes or configuration fingerprints are never joined into one line.</p>' + metricDefinitions().map(chart).join("") + '</section><section class="p5-section" id="p5-keywords"><h2>Scope</h2>' + watchSetSummary() + '</section><section class="p5-section" id="p5-recognition"><div class="p5-charthead"><div><h2>This run</h2><p class="p5-muted">One model failing never overwrites another model's evidence. You can inspect and retry each probe on its own.</p></div>' + requestButton("Test newly added models only", "new-models") + '</div>' + runSummary() + '</section><section class="p5-section" id="p5-monitoring"><div class="p5-charthead"><div><h2>Scheduled monitoring</h2><p class="p5-muted">A scheduled task reuses the same measurement execution path and records every run it produces.</p></div>' + requestButton("Set up scheduled monitoring", "schedule") + '</div><div class="p5-grid">' + taskSummary() + '</div></section>';
