...
function eventTitle(project: MonitoringProject, condition: MonitoringNotificationCondition): string {
  ...
  const zh: Record<MonitoringNotificationCondition, string> = {
    brand_disappeared: "The brand disappeared from monitored answers",
    competitor_appeared: "A new competitor started appearing in place of the brand",
    official_citation_added: "AI newly cited the official site",
    recommendation_changed: "The brand's recommendation result changed",
    run_completed: "The monitoring task completed",
    run_failed: "The monitoring task failed",
  };
...
function eventMessage(event: MonitoringEvent, task: MonitoringTask, project: MonitoringProject): string {
  const language = project.defaultLanguage.toLocaleLowerCase().split("-")[0];
  if (language === "zh") {
    return `Task: ${task.name}\nEvents: ${event.occurrenceCount}\nQuestion: ${event.promptIds.length}\nModel: ${event.models.join(", ") || "None"}\nOpen the CiteGEO workbench to see the matching question and the raw answer.`;
  }
  return `Task: ${task.name}\nOccurrences: ${event.occurrenceCount}\nQuestions: ${event.promptIds.length}\nModels: ${event.models.join(", ") || "none"}\nOpen the CiteGEO workspace to inspect the questions and original answers.`;
}
