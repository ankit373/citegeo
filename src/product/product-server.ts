...
async function handle(req: IncomingMessage, res: ServerResponse, services: ProductServices): Promise<void> {
  ...
  const { projects, catalog, selections, baselines, recognition, reports, insights, signals, crawlerLog, watchSets, measurements, stats, schedules, topics, promptRuns } = services;
  ...
  if (await handleTopicApi({ method, route, send: json, topics, runs: promptRuns, ask: services.ask, readJson: body })) return;
